...
shareDir = '/home/em/sworble/share/'  #hard coded share directory. You will want to change it accordingly.

#***********************************************************
# getDirsBelow(directory) takes a path to a directory and returns a list of
# paths to subdirectories below the top directory.

def getDirsBelow(directory):
    dirList = []
...
MasterFileList = [x for x in getFilesBelow(shareDir)] #list of all files in share directory by path

MasterDirList = [x for x in getDirsBelow(shareDir)] #list of all dirs in share directory by path

HashToFileDict = {hashFile(x) : x for x in MasterFileList} # { hash: path .. }

FileToHashDict = {v : k for (k,v) in HashToFileDict.iteritems()} # { path: hash .. }  
...
def hashDir(pathtodir):
    SHA1hash = hashlib.sha1()
    HashList = []
    fb = getFilesBelow(pathtodir) # generates a list of all the files below the dir
    for f in fb:
      HashList.append(FileToHashDict[f]) # this gets the hash for file 'f' and adds it to the list
...
def HashToPathDict():
    for d in MasterDirList:
      HashToFileDict[hashDir(d)] = d
    return HashToFileDict
